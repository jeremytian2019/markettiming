...
class Broker:
    ...
    def order_open(self, exercise_price):    # 开仓买入或者卖出
        ...
        if len(self.total_position) == 0:  # 回测起始日
            ...
            self.market_value.append(exercise_price * abs(self.ctx.bar.signal))
            self.open_date.append(self.ctx.time)
            self.open_price.append(exercise_price)
        else:      # 非回测起始日
            if self.total_position[-1] == 0:  # 上个交易日没有持仓头寸
                order_position = max(1, int(sum(self.ret) / exercise_price)) * self.ctx.bar.signal
                self.ret.append(order_position * (self.ctx.bar.close - exercise_price - self.commission - self.slippage))
                self.total_position.append(order_position)
                self.order_position.append(order_position)
                self.open_date.append(self.ctx.time)
                self.open_price.append(exercise_price)
                self.market_value.append(abs(order_position) * self.ctx.bar.close)
            else:   # 上个交易日有持仓头寸
                order_position = int(sum(self.ret) / exercise_price) - abs(self.total_position[-1])
                if order_position > 0:  # 增仓
                    self.ret.append(order_position * (self.ctx.bar.signal * (self.ctx.bar.close - exercise_price) - \
                                                      self.commission - self.slippage) + \
                                    self.total_position[-1]*(self.ctx.bar.close - self.ctx.bar.lastclose))
                    self.open_date.append(self.ctx.time)
                    self.open_price.append(exercise_price)
                    self.order_position.append(order_position * self.ctx.bar.signal)
                    self.total_position.append(order_position * self.ctx.bar.signal + self.total_position[-1])
                    ...
                    self.market_value.append(self.total_position[-1] * self.ctx.bar.close)

    def order_close(self, exercise_price):    # 空仓或者平仓
